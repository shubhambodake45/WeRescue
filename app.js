...
app.get("/setProfileAttributes", function(req, res) {
  res.render("setProfileAttributes", {
    ejs_googleUser: localGoogleUser.currentUser
  });
});
...
app.get("/healthOfficialLogin", function(req, res) {
  res.render("healthOfficialLogin", {
    messages: req.flash("status")
  });
});
...
app.get("/healthOfficialInterface", function(req, res) {
  Offical.findOne({
    ...
    if (err) {
      ...
      var casesByDatePerAnimal = {};
      var alertValues = [0, 0, 0];
      var totalPost = 0;
      var totalPostReviewed = 0;
      var totalPostReviewedByCurrOffical = 0;
      var totalRedAlerts = 0;
      var animalTypeCount = {};
      var animalNameCount = {};
      var cityCount = {};
      var stateCount = {};
      ...
      Post.find(function(err, foundPost) {
        if (!err) {
          ...
          for (let i = 0; i < postReviewedByCurrOfficial.length; i++) {
            if (postReviewedByCurrOfficial[i].emergencyAlert === "green") {
              alertValues[0] = alertValues[0] + 1;
            }
            if (postReviewedByCurrOfficial[i].emergencyAlert === "yellow") {
              alertValues[1] = alertValues[1] + 1;
            }
            if (postReviewedByCurrOfficial[i].emergencyAlert === "red") {
              alertValues[2] = alertValues[2] + 1;
            }

            if (!animalTypeCount[postReviewedByCurrOfficial[i].animalType]) {
              animalTypeCount[postReviewedByCurrOfficial[i].animalType] = 1;
            } else {
              animalTypeCount[postReviewedByCurrOfficial[i].animalType]++;
            }

            if (!animalNameCount[postReviewedByCurrOfficial[i].animalName]) {
              animalNameCount[postReviewedByCurrOfficial[i].animalName] = 1;
            } else {
              animalNameCount[postReviewedByCurrOfficial[i].animalName]++;
            }

            if (!cityCount[postReviewedByCurrOfficial[i].city]) {
              cityCount[postReviewedByCurrOfficial[i].city] = 1;
            } else {
              cityCount[postReviewedByCurrOfficial[i].city]++;
            }

            if (!stateCount[postReviewedByCurrOfficial[i].state]) {
              stateCount[postReviewedByCurrOfficial[i].state] = 1;
            } else {
              stateCount[postReviewedByCurrOfficial[i].state]++;
            }

            let caseDate = postReviewedByCurrOfficial[i].dateAndTime.substring(0, postReviewedByCurrOfficial[i].dateAndTime.indexOf(","));
            if (!casesByDate[caseDate]) {
              casesByDate[caseDate] = 1;
            } else {
              casesByDate[caseDate]++;
            }

            if (!casesByDatePerAnimal[caseDate]) {
              // console.log("*" + postReviewedByCurrOfficial[i].animalType);
              casesByDatePerAnimal[caseDate] = {
                "mammal": 0,
                "bird": 0,
                "fish": 0,
                "reptile": 0,
            ...
            casesByDatePerAnimal[caseDate][postReviewedByCurrOfficial[i].animalType]++;
          }

          console.log(animalTypeCount);
          console.log(animalNameCount);
          console.log(cityCount);
          console.log("Per Animal");
          console.log(casesByDatePerAnimal);

          var animalTypeCountSorted = [];
          for (var x in animalTypeCount) {
            animalTypeCountSorted.push([x, animalTypeCount[x]]);
          }
          animalTypeCountSorted.sort(function(a, b) {
            return b[1] - a[1];
          });

          var animalNameCountSorted = [];
          for (var x in animalNameCount) {
            animalNameCountSorted.push([x, animalNameCount[x]]);
          }
          animalNameCountSorted.sort(function(a, b) {
            return b[1] - a[1];
          });

          var cityCountSorted = [];
          for (var x in cityCount) {
            cityCountSorted.push([x, cityCount[x]]);
          }
          cityCountSorted.sort(function(a, b) {
            return b[1] - a[1];
          });

          var stateCountSorted = [];
          for (var x in stateCount) {
            stateCountSorted.push([x, stateCount[x]]);
          }
          stateCountSorted.sort(function(a, b) {
            return b[1] - a[1];
          });

          // TODO sort this properly by date ascending order
          var casesByDateSorted = [];
          for (var x in casesByDate) {
            casesByDateSorted.push([x, casesByDate[x]]);
          }
          casesByDateSorted.sort(function(a, b) {
            return new Date(b) - new Date(a);
          });

          var casesByDatePerAnimalSorted = [];
          for (var x in casesByDatePerAnimal) {
            casesByDatePerAnimalSorted.push([x, casesByDatePerAnimal[x]]);
          }
          casesByDatePerAnimalSorted.sort(function(a, b) {
            return new Date(b) - new Date(a);
          });

          console.log("Sorted Types");
          console.log(animalTypeCountSorted);
          console.log(animalNameCountSorted);
          console.log(cityCountSorted);
          console.log(stateCountSorted);
          console.log(casesByDateSorted);
          console.log(casesByDatePerAnimalSorted);

          totalPostReviewed = totalPost - unReviwedPost.length;
          totalPostReviewedByCurrOffical = postReviewedByCurrOfficial.length;
          totalRedAlerts = alertValues[2];
          ...
          res.render("healthOfficialInterface", {
            ...
            totalPost: totalPost,
            totalPostReviewed: totalPostReviewed,
            totalPostReviewedByCurrOffical: totalPostReviewedByCurrOffical,
            totalRedAlerts: totalRedAlerts,
            animalTypeCountSorted: animalTypeCountSorted,
            animalNameCountSorted: animalNameCountSorted,
            cityCountSorted: cityCountSorted,
            stateCountSorted: stateCountSorted,
            casesByDateSorted: casesByDateSorted,
            casesByDatePerAnimalSorted: casesByDatePerAnimalSorted,
          });
        } else {
          console.log(err);
        }
